...
internals.schema.strategy = Joi.object({
    ...
    verify: Joi.object({
        aud: Joi.array().items(Joi.string(), Joi.object().type(RegExp)).min(1).single().allow(false).required(),
...
internals.implementation = function (server, options) {
    ...
    return {
        authenticate: async function (request, h) {
            ...
            if (settings.verify) {
                try {
                    internals.verify(result.artifacts, settings);
                }
                catch (err) {
                    return h.unauthenticated(unauthorized(err.message), result);
                }
...
internals.verify = function (artifacts, options) {

    for (const key of artifacts.keys) {
        try {
            return Token.verify(artifacts, key, options.verify);
        }
        catch (err) {
    ...
    throw new Error('Failed validating token');
};
