...
exports.generate = function (payload, secret, options = {}) {
    ...
    const value = `${Utils.b64stringify(header, options.encoding)}.${Utils.b64stringify(content, options.encoding)}`;
...
exports.decode = function (token, options = {}) {
    ...
    if (parts.length === 3) {
        ...
        artifacts.decoded.header = internals.b64parse(artifacts.raw.header, options.encoding);
    }
    ...
    artifacts.decoded.payload = internals.b64decode(artifacts.raw.payload, options.encoding);
...
exports.headless = function (options) {
    ...
    if (typeof headless === 'object') {
        ...
        return {
            ...
            raw: Buffer.from(JSON.stringify(headless), options.encoding).toString('base64'),
            decoded: headless
        };
    }
    ...
    return {
        [internals.headless]: true,
        raw: headless,
        decoded: internals.b64parse(headless, options.encoding)
    };
};
...
internals.b64decode = function (string, encoding = 'utf8') {

    return Buffer.from(string, 'base64').toString(encoding);
};


internals.b64parse = function (string, encoding = 'utf8') {

    return Bourne.safeParse(internals.b64decode(string, encoding));
};
